...
public class CMStatusBarUtil {
    private CMStatusBarUtil() {
    }
    ...
    public static void setNavigationBarColor(Activity activity, @ColorInt int color, @IntRange(from = 0, to = 255) int statusBarAlpha) {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                //底部导航栏
                window.setNavigationBarColor(calculateStatusColor(color, statusBarAlpha));
            }
        } catch (Exception e) {
//            LogUtil.e(e.getMessage());
    ...
    @TargetApi(Build.VERSION_CODES.M)
    public static void setNavigationLightMode(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
    ...
    @TargetApi(Build.VERSION_CODES.M)
    public static void setNavigationDarkMode(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            activity.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_VISIBLE);
        }
    }
    ...
    @TargetApi(Build.VERSION_CODES.M)
    public static void setStatusNavigationBothLightMode(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
    ...
    @TargetApi(Build.VERSION_CODES.M)
    public static void setStatusColor(Activity activity, boolean isTransparent, boolean isDarkText, @ColorInt int bgColor) {
        if (isDarkText) {
            setMIUIStatusBarDarkIcon(activity, true);
            setMeizuStatusBarDarkIcon(activity, true);
        } else {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            decorView.setSystemUiVisibility((isTransparent ? View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN : 0) | (isDarkText ? View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR : 0));
            setColor(activity, bgColor, 0);
        }
    }
...
//                LogUtil.d(e.getMessage());
     ...
     * 设置状态栏全透明
     *
     * @param window window {@link Window}
     * @return true, 则可以设置状态栏透明，false，则不支持状态栏透明
     */
